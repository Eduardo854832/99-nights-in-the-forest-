-- Universal Utility (Phase 1 / 0.9.0)
-- Principais adições: Lazy Panels, Temas, Perfis, Keybind Manager, Auto Sprint, Logger, Histórico TP, API pública expandida.
-- Nova API: Core.API.{RegisterCommand,RegisterLazyPanel,RegisterKeybind,Log,PanicAll,GetState}
-- Comando base: /uu <command> ...
-- Comandos: theme, profile, keybind, sprint, debug, panic, export, import, help
-- ATENÇÃO: Este arquivo substitui completamente a versão 0.6.x
...
-- ==== Persistência com Flush em Lote (0.8.0+) ====
local Persist = {}
...
-- ==== Internacionalização (mesmo baseline + chaves extras 0.8/0.9 se necessário) ====
local Lang = {}
Lang.data = {
    pt = {
...
Core._tpHistory = Persist.get("tp_history",{ }) -- last 5
Core._state = {
    version = VERSION,
    started = os.clock(),
...
function Core.API_PanicAll()
    -- Desativar sprint, overlay, fly (legacy se existir), noclip, custom time
...
function UI.applyLanguage()
    for _, data in ipairs(UI._translatables) do
        if data.instance and data.instance.Parent and data.instance:IsA("TextLabel") or data.instance:IsA("TextButton") then
...
function UI.createRoot()
    ...
    mini.BackgroundColor3 = Themes.dark.header
    ...
    miniTip.TextSize = 12
    miniTip.TextColor3 = Themes.dark.textDim
    ...
    floating.BackgroundColor3 = Themes.dark.bg
    ...
    hideBtn.BackgroundColor3 = Themes.dark.accent
...
function UI.buildPanelButton(key)
    ...
    holder.BackgroundColor3 = Themes.dark.panel
    ...
    header.BackgroundColor3 = Themes.dark.accent
...
local function UI_Label(parent, text, isKey)
    ...
    lbl.TextColor3 = Themes.dark.textDim
...
local function UI_Button(parent, textKeyOrRaw, isKey, callback)
    ...
    b.BackgroundColor3 = Themes.dark.accent
    b.TextColor3 = Color3.new(1,1,1)
    b.Font = Enum.Font.GothamBold
    b.TextSize = 13
...
local function UI_Toggle(parent, labelKey, persistKey, default, callback)
    ...
    lbl.TextSize = 14
    lbl.TextXAlignment = Enum.TextXAlignment.Left
    lbl.TextColor3 = Themes.dark.text
    lbl.Text = L(labelKey)
    lbl.Parent = holder
    mark(lbl,labelKey)
...
local function UI_Slider(parent, labelKey, persistKey, minVal, maxVal, defaultVal, step, callback)
    ...
    bar.BackgroundColor3 = Themes.dark.accent
    bar.Parent = holder
    ...
    fill.BackgroundColor3 = Themes.dark.highlight
    fill.Parent = bar
...
RunService.RenderStepped:Connect(function()
    ...
    if now - metricsState.lastTick >= 1 then
        ...
        metricsState.ping = Stats.Network.ServerStatsItem["Data Ping"] and Stats.Network.ServerStatsItem["Data Ping"]:GetValue() or -1
...
-- ==== Sprint System (Auto Sprint 0.9.0) ====
local Sprint = {}
...
function Sprint.apply()
    ...
    local base = Persist.get("walkspeed_value", hum.WalkSpeed)
    if Sprint.isActive then
        local bonus = Persist.get("sprint_bonus",8)
        hum.WalkSpeed = base + bonus
    end
end
...
Core.API_RegisterLazyPanel("PANEL_MOVEMENT", function(panel)
    ...
    UI_Slider(panel,"SLIDER_WALKSPEED","walkspeed_value",4,64, hum and hum.WalkSpeed or 16,1,function(v)
        local h = Util.getHumanoid(); if h then h.WalkSpeed = v end
    end)
    UI_Slider(panel,"SLIDER_JUMPPOWER","jumppower_value",25,150, hum and hum.JumpPower or 50,1,function(v)
        local h = Util.getHumanoid(); if h and h.UseJumpPower ~= false then h.JumpPower = v end
    end)
...
Core.API_RegisterLazyPanel("PANEL_CAMERA", function(panel)
    ...
    UI_Toggle(panel,"TOGGLE_SHIFTLOCK","camera_shiftlock",state.shiftLock,function(on) state.shiftLock=on end)
    UI_Toggle(panel,"TOGGLE_SMOOTH","camera_smooth",state.smooth,function(on) state.smooth=on end)
...
-- Extras Panel (positions, noclip, overlay, ambience)
    ...
    RunService.Heartbeat:Connect(function()
        if active then
            ...
            if hum and root then
                ...
                if bodyVel then
                    bodyVel.Velocity = dir.Unit * (dir.Magnitude>0 and speed or 0)
                end
            end
        end
    end)
...
Core.API_RegisterLazyPanel("PANEL_DEBUG", function(panel)
    ...
    box.BackgroundColor3 = Themes.dark.panel
    ...
    local missingList = {}
    -- gather all translatable keys referenced
    local used = {}
...
-- Missing Keys Panel (separado se quiser abrir isoladamente)
...
-- Perf Overlay (global single build)
local function buildPerfOverlay()
    ...
    box.BackgroundColor3 = Themes.dark.header
    ...
    local acc,frames,last = 0,0,tick()
    RunService.RenderStepped:Connect(function()
        frames+=1
        local now = tick()
        acc += (now-last)
...
ensureLanguage(function()
    ...
    -- Provide a dummy KEYBINDS panel builder quickly
    ...
    -- Build overlay
    buildPerfOverlay()
    -- Apply theme after building (persisted)

...
-- ==== Themes ====
local Themes={
    dark={bg=Color3.fromRGB(20,22,30),header=Color3.fromRGB(30,34,46),panel=Color3.fromRGB(28,32,42),
          accent=Color3.fromRGB(55,65,90),text=Color3.fromRGB(235,235,245),textDim=Color3.fromRGB(200,205,215),
...
function UI.createRoot()
    ...
    floating.Position=loadMenuPos(UDim2.new(0.05,0,0.25,0))
    ...
    list.Changed:Connect(function(p)
        if p=="AbsoluteContentSize" then scroll.CanvasSize=UDim2.new(0,0,0,list.AbsoluteContentSize.Y+20) end
    end)

    Util.draggable(floating, header, function(pos)
        saveMenuPos(pos)
        if not floating.Visible then mini.Position=pos; saveMenuPos(mini.Position) end
    end)
    Util.draggable(mini, mini, function(pos) saveMenuPos(pos) end)
...
-- ==== Sprint (funcionalidade interna apenas via comando) ====
local Sprint={}
...
-- ==== Profiles ====
local Profiles={}
local profileKeys={"walkspeed_value","jumppower_value","camera_fov","camera_sens","camera_shiftlock","camera_smooth","overlay_visible","sprint_enabled","sprint_bonus","sprint_hold_time","sprint_hold_mode"}
...
Core.API_RegisterCommand("debug","Open logger panel",function()
    if UI._panelButtons["PANEL_DEBUG"] then UI._panelButtons["PANEL_DEBUG"]:Activate() end
end)
...
ensureLanguage(function()
    ...
    for _,k in ipairs(order) do UI.buildPanelButton(k) end
    buildPerfOverlay()

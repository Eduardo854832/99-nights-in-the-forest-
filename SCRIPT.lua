...
local firstLanguageChosen = false
local function T(k)
...
--------------------------------------------------
-- Config (mantido mínimo para keybinds & valores, sem UI de salvar)
--------------------------------------------------
local Config = {
    data = {
        lang = currentLang,
...
local function buildUI()
    ...
    Factory:Toggle(pgVisual, T("toggle_esp_names"), Systems.ESP.ShowNames, function(st) Systems.ESP.ShowNames=st end)
    Factory:Toggle(pgVisual, T("toggle_esp_distance"), Systems.ESP.ShowDistance, function(st) Systems.ESP.ShowDistance=st end)
    Factory:Toggle(pgVisual, T("toggle_esp_tracers"), Systems.ESP.Tracers, function(st) Systems.ESP.Tracers=st end) -- placeholder
    Factory:Slider(pgVisual, T("slider_esp_distance"), 50, 2000, Systems.ESP.DistanceLimit, function(v)
    ...
    Factory:Slider(pgPlayer, T("slider_flight_speed"), 20, 400, Systems.Flight.Speed, function(v)
    ...
    Factory:Slider(pgPlayer, T("slider_walkspeed"), 8, 200, (safeHumanoid() and safeHumanoid().WalkSpeed) or 16, function(v)
        local hum=safeHumanoid(); if hum then hum.WalkSpeed=v end
    end)
    Factory:Slider(pgPlayer, T("slider_jumppower"), 10, 200, (safeHumanoid() and safeHumanoid().JumpPower) or 50, function(v)
        local hum=safeHumanoid(); if hum then hum.JumpPower=v end
    end)
    Factory:Slider(pgPlayer, T("slider_gravity"), 0, 400, workspace.Gravity, function(v)
        workspace.Gravity = v
    end)
    ...
    Factory:Toggle(pgUtil, T("toggle_antiafk"), Systems.AntiAFK.Enabled, function(st)
        if st then Systems.AntiAFK:Enable() else Systems.AntiAFK:Disable() end
    end)
    -- REMOVIDO: Botões Copy Position / Copy CFrame conforme pedido

    --------------------------------------------------
    -- SETTINGS (Keybinds, Tema, Plugins)
    --------------------------------------------------
    ...
    -- (Removido) Botões de linguagem aqui. A escolha agora é apenas na inicialização.
...
--------------------------------------------------
-- Prompt Inicial de Idioma (aparece somente ao iniciar)
--------------------------------------------------
local function showLanguagePrompt()
    if firstLanguageChosen then
        buildUI()
        return
    end
    local gui = Instance.new("ScreenGui")
    gui.Name="UHLanguagePrompt"
    gui.ResetOnSpawn=false
    gui.ZIndexBehavior=Enum.ZIndexBehavior.Global
    gui.Parent = LocalPlayer:WaitForChild("PlayerGui")

    local frame = create("Frame",{Size=UDim2.fromOffset(360,160),Position=UDim2.new(0.5,-180,0.5,-80),
        BackgroundColor3=THEME.Background,BorderSizePixel=0})
    create("UICorner",{CornerRadius=UDim.new(0,10)}).Parent=frame
    applyStroke(frame)
    frame.Parent=gui

    local lbl = create("TextLabel",{Text="Select Language / Selecione o Idioma",Font=Enum.Font.GothamBold,TextSize=18,
        TextColor3=THEME.TextPrimary,BackgroundTransparency=1,Size=UDim2.new(1,-20,0,40),Position=UDim2.fromOffset(10,10)})
    lbl.Parent=frame

    local function langButton(txt, lang, pos)
        local b = create("TextButton",{Text=txt,Font=Enum.Font.GothamSemibold,TextSize=16,TextColor3=THEME.TextPrimary,
            BackgroundColor3=THEME.BackgroundAlt,AutoButtonColor=false,Size=UDim2.new(0.5,-20,0,50),
            Position=UDim2.new(pos,10,0,70)})
        create("UICorner",{CornerRadius=UDim.new(0,8)}).Parent=b
        applyStroke(b)
        b.Parent=frame
        b.MouseEnter:Connect(function() b.BackgroundColor3=THEME.Accent end)
        b.MouseLeave:Connect(function() b.BackgroundColor3=THEME.BackgroundAlt end)
        b.MouseButton1Click:Connect(function()
            currentLang = lang
            firstLanguageChosen = true
            gui:Destroy()
            buildUI()
        end)
    end
    langButton("Português","pt",0)
    langButton("English","en",0.5)
end

...
-- Novo: draggable com callback de soltar
...
function UI.applyLanguage()
    ...
    if UI.MiniButton then
        ...
        UI.MiniTip.Text = L("MINI_TIP")
    end
end

-- Helper para persistir posição
local function saveMenuPosFromUDim2(pos)
    Persist.set("ui_menu_pos", {
        sx = pos.X.Scale, x = pos.X.Offset,
        sy = pos.Y.Scale, y = pos.Y.Offset
    })
end
local function loadMenuPosition(defaultPos)
    local data = Persist.get("ui_menu_pos", nil)
    if data then
        return UDim2.new(data.sx or 0, data.x or 0, data.sy or 0, data.y or 0)
    end
    return defaultPos
end
...
function UI.createRoot()
    ...
    -- Mini (quadrado) handle – aparece quando menu está oculto
    local mini = Instance.new("TextButton")
    mini.Name = "MiniMenuHandle"
    ...
    -- Drag principal (pelo cabeçalho)
    Util.draggable(floating, header, function(pos)
        saveMenuPosFromUDim2(pos)
        -- manter mini handle sincronizado se estiver oculto
        if not floating.Visible then
            mini.Position = UDim2.new(pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
            saveMenuPosFromUDim2(mini.Position)
        end
    end)

    -- Drag do mini quadrado
    Util.draggable(mini, mini, function(pos)
        -- Atualiza posição persistente para quando reabrir
        saveMenuPosFromUDim2(pos)
    end)
    ...
    hideBtn.MouseButton1Click:Connect(function()
        ...
        mini.Visible = true
        -- posicionar mini no mesmo local do menu para continuidade
    ...
    UserInputService.InputBegan:Connect(function(input,gp)
        ...
        if input.KeyCode == Enum.KeyCode.F4 then
            if floating.Visible then
                -- ocultar menu, mostrar quadrado
                floating.Visible = false
                mini.Visible = true
                mini.Position = floating.Position
                saveMenuPosFromUDim2(mini.Position)
            else
                -- se mini visível, reabrir
...
ensureLanguage(function()
    ...
    Core.register({
        ...
        init = function(ctx)
            ...
            -- Restaurar posição overlay se salva
        ...
        init=function()
            ...
            local function buildFlyGUI()
                ...
                mine.MouseButton1Down:Connect(function()
                    if speeds == 1 then
                        speedLbl.Text = 'min 1'
                        task.delay(1,function() speedLbl.Text = tostring(speeds) end)
                    else
                        speeds -= 1
                ...
                mini.MouseButton1Click:Connect(function()
                    up.Visible=false; down.Visible=false; onof.Visible=false; plus.Visible=false; speedLbl.Visible=false; mine.Visible=false; mini.Visible=false; mini2.Visible=true
                end)
                mini2.MouseButton1Click:Connect(function()
                    up.Visible=true; down.Visible=true; onof.Visible=true; plus.Visible=true; speedLbl.Visible=true; mine.Visible=true; mini.Visible=true; mini2.Visible=false
                end)
                flyGuiInstance = main
            end
            toggleBtn = UI.button(panelFly,"FLYGUI_OPEN",true,function()
                if not isOpen then buildFlyGUI(); isOpen = true; if toggleBtn then toggleBtn.Text = L("FLYGUI_CLOSE") end
                else
